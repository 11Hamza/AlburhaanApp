...
import { exchangeMicrosoftCode, getMicrosoftUserProfile } from '@/lib/oauth';
...
export async function GET(request) {
  try {
    ...
    if (error) {
      ...
      errorUrl.searchParams.set('provider', 'microsoft');
    ...
    // Get user profile
    const profileResult = await getMicrosoftUserProfile(tokenResult.data.access_token);
    ...
    if (ssoAccount) {
      ...
      if (searchResult.success && searchResult.data && searchResult.data.length > 0) {
        ...
        registerUrl.searchParams.set('provider', 'microsoft');
    ...
    return Response.redirect('/?error=callback_failed&provider=microsoft');
  }
}
